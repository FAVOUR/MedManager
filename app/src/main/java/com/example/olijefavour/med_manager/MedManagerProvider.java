...
public class MedManagerProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        if (name == null) {
            throw new IllegalArgumentException("Medication requires a name");
        }

        // Check that the frequency is valid
        Integer frequency = values.getAsInteger(MedManagerEntry.COLUMN_FREQUENCY_INTERVAL);
        if (frequency == null) {
            throw new IllegalArgumentException("Medication requires valid Interval");
        }

//        // If the weight is provided, check that it's greater than or equal to 0 kg
//        Integer weight = values.getAsInteger(MedManagerEntry.COLUMN_PET_WEIGHT);
